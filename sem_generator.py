...
class SEMGenerator:
    ...
    def generate_custom(self, n):
        ...
        for l in range(n):
            ...
            fill_color = random.choice(scfg.COLORS) + ':' + random.choice(scfg.COLORS) if random.random() < 0.2 else '#FFFFFF'
            ...
            self.dot = graphviz.Digraph(comment=f'SEM_{l}', format='png', graph_attr={'splines': edges_shape}, engine='neato')
            ...
            for ind, lv in enumerate(self.latent_variables):
                ...
                occupied.append((pos_x, pos_y)); occupied.append((pos_x + 1, pos_y)); occupied.append((pos_x - 1, pos_y))
                occupied.append((pos_x, pos_y - 1)); occupied.append((pos_x, pos_y + 1)); occupied.append((pos_x + 1, pos_y + 1))
                occupied.append((pos_x - 1, pos_y + 1)); occupied.append((pos_x + 1, pos_y - 1)); occupied.append((pos_x - 1, pos_y - 1))
                ...
                self.dot.node(f'{lv}_construct', lv, pos=pos, style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)
                ...
                for ov in self.obs_vars[ind]:
                    ...
                    self.dot.node(f'{ov}_item', ov, pos=pos, style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)
                    
                    if random.random() >= 0.1:
                            self.dot.edge(f'{lv}_construct', f'{ov}_item', label=self._create_edge_label(), fontsize=pc_sz, 
                                          arrowhead=arrow, arrowtail=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname)
                    else:
            ...
            for edges in self.lv_edges:
                ...
                if random.random() >= 0.1:
                    self.dot.edge(f'{edges[0]}_construct', f'{edges[1]}_construct', dir=edges[2], label=self._create_edge_label(), fontsize=pc_sz, 
            ...
            if self.item_edges:
                for edges in self.ov_edges:
                    if random.random() >= 0.1:    
                        self.dot.edge(f'{edges[0]}_item', f'{edges[1]}_item', label=self._create_edge_label(), fontsize=pc_sz, 
                                      arrowhead=arrow, arrowtail=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname)
                    else:
            ...
            if random.random() >= 0.7:
                ...
                if random.random() >= 0.9:
                    self.dot.attr(label=f'\n\n{random.choice(random_list)}', fontsize=fontsize, labelloc=label_pos[0], labeljust=label_pos[1])
                elif random.random() >= 0.8:
                    self.dot.attr(label=f'\n\n{random.choice(random_list)}\n{random.choice(random_list)}', fontsize=fontsize, labelloc=label_pos[0], labeljust=label_pos[1])
                elif random.random() >= 0.65:
                    self.dot.attr(label=f'\n{random.choice(random_list)}\n{random.choice(random_list)}\n{random.choice(random_list)}', fontsize=fontsize, labelloc=label_pos[0], labeljust=label_pos[1])
                elif random.random() >= 0.5:
                    self.dot.attr(label=f'\n{random.choice(random_list)}\n{random.choice(random_list)}\n{random.choice(random_list)}\n{random.choice(random_list)} {random.choice(random_list)}', fontsize=fontsize, labelloc=label_pos[0], labeljust=label_pos[1])
    ...
    def generate_random(self, n):
        ...
        lv_positions = [f'{random.randint(-n_vars, n_vars)},{random.randint(-n_vars, n_vars)}!' for _ in self.latent_variables]
        ov_positions = [[f'{random.randint(-n_vars, n_vars)},{random.randint(-n_vars, n_vars)}!' for _ in self.obs_vars[i]] for i in range(len(self.obs_vars))]
        ...
        for l in range(n):
            ...
            items = random.choice(scfg.ITEM_SHAPE)
            lv_shapes = random.choice(scfg.LV_SHAPE)
            fill_color = random.choice(scfg.COLORS) + ':' + random.choice(scfg.COLORS) if random.random() < 0.5 else '#FFFFFF'
            fill_angle = str(random.randint(0, 360))
            error = random.choice(scfg.ERRORS)
            error_shape = random.choice(scfg.ERROR_SHAPE)
            fontname = random.choice(scfg.FONTNAMES)
            self.log = self.log.append({'Filename': f'{self.json_path}_{l}_random', 'Number_constructs': len(self.latent_variables), 
                       'Number_items': sum(self.amount_obs_vars), 'Arrow': arrow, 'Edge_attr': edges, 
                       'Penwidth': width, 'Arrow_size': arrow_sz, 'Fontsize': pc_sz, 'Item_shape': items, 'Construct_shape': lv_shapes, 
                       'Fill_color': fill_color, 'Fill_angle': fill_angle, 'Errors': error, 'Error_shape': error_shape}, ignore_index=True)
            ...
            for ind, lv in enumerate(self.latent_variables):
                ...
                self.dot.node(f'{lv}_construct', lv, pos=lv_positions[ind], style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)
                ...
                for j, ov in enumerate(self.obs_vars[ind]):
                    ...
                    self.dot.node(f'{ov}_item', ov, style='filled', pos=ov_positions[ind][j], fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)
    ...
    def generate_circo(self, n):
        ...
        for l in range(n):
            ...
            items = random.choice(scfg.ITEM_SHAPE)
            lv_shapes = random.choice(scfg.LV_SHAPE)
            fill_color = random.choice(scfg.COLORS) + ':' + random.choice(scfg.COLORS) if random.random() < 0 else '#FFFFFF'
            ...
            fontname = random.choice(scfg.FONTNAMES)
            label_pos = random.choice(scfg.LABEL_POS)
            self.log = self.log.append({'Filename': f'{self.json_path}_{l}_circo', 'Number_constructs': len(self.latent_variables), 
                       'Number_items': sum(self.amount_obs_vars), 'Arrow': arrow, 'Edge_attr': edges_shape, 
                       'Penwidth': width, 'Arrow_size': arrow_sz, 'Fontsize': pc_sz, 'Item_shape': items, 'Construct_shape': lv_shapes, 
                       'Fill_color': fill_color, 'Fill_angle': fill_angle, 'Errors': error, 'Error_shape': error_shape}, ignore_index=True)
            
            self.dot = graphviz.Digraph(comment=f'SEM_{l}', format='png', graph_attr={'splines': edges_shape}, engine='circo')
            ...
            for ind, lv in enumerate(self.latent_variables):
                if cluster and (random.random() < 0.33):
                    ...
                    with self.dot.subgraph(name=f'cluster_{lv}') as c:
                        ...
                        c.node(f'{lv}_construct', lv, style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)

                        c.attr('node', shape=items)
                        for ov in self.obs_vars[ind]:
                            
                            
                            c.node(f'{ov}_item', ov, style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname)
                            if random.random() >= 0.2:
                                c.edge(f'{lv}_construct', f'{ov}_item', label=self._create_edge_label(), fontsize=pc_sz, 
                                              arrowhead=arrow, arrowtail=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname, color=edge_color)
                            else:
                            ...
                            if (error_shape in ['circle', 'plaintext']) and (error != 'none'):
                                ...
                                c.node(f'{e_name}_error', e_name, shape=error_shape, style='filled', 
                                             fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname)
                                if random.random() >= 0.1:
                                    c.edge(f'{e_name}_error', f'{ov}_item', label='1  ', fontsize=pc_sz, arrowhead=arrow, 
                                                     arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname)
                                else:
                    ...
                    self.dot.node(f'{lv}_construct', lv, style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)

                    self.dot.attr('node', shape=items)
                    for ov in self.obs_vars[ind]:
                        self.dot.node(f'{ov}_item', ov, style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname)
                        if random.random() >= 0.2:
                            self.dot.edge(f'{lv}_construct', f'{ov}_item', label=self._create_edge_label(), fontsize=pc_sz, 
                                          arrowhead=arrow, arrowtail=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname, color=edge_color)
                        else:
                        ...
                        if (error_shape in ['circle', 'plaintext']) and (error != 'none'):
                            ...
                            if random.random() >= 0.1:
                                self.dot.edge(f'{e_name}_error', f'{ov}_item', label='1  ', fontsize=pc_sz, arrowhead=arrow, 
                                                 arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname, color=edge_color)
                            else:
                                self.dot.edge(f'{e_name}_error', f'{ov}_item', arrowhead=arrow, 
                                             arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname, color=edge_color)
    ...
    def generate_dot(self, n):
        ...
        for l in range(n):
            ...
            self.dot = graphviz.Digraph(comment=f'SEM_{l}', format='png', graph_attr={'splines': edges_shape, 'rankdir': rankdir}, engine='dot')
                    ...
                    with self.dot.subgraph(name=f'cluster_{lv}') as c:
                        ...
                        for ov in self.obs_vars[ind]:
                            ...
                            if (error_shape in ['circle', 'plaintext']) and (error != 'none'):
                                ...
                                e_name = f'e{curr_h_pos}'
                                if random.random() >= 0.1:
                                    self.dot.edge(f'{e_name}_error', f'{lv}_construct', label='1 ', fontsize=pc_sz, arrowhead=arrow, 
                                                     arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname)
                                else:
        ...
        for l in range(n):
            ...
            for lv in self.latent_variables:
                for ov in observed_vars:
                    ...
                    if random.random() >= 0.1:
                        if random.random() >= 0.2:
                            self.dot.edge(f'{lv}_construct', f'{ov}_item', dir='forward', label=self._create_edge_label(), fontsize=pc_sz, 
                                          arrowhead=arrow, arrowtail=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname, color=edge_color)
                        else:
                            self.dot.edge(f'{lv}_construct', f'{ov}_item', dir='both', label=self._create_edge_label(), fontsize=pc_sz, 
                                          arrowhead=arrow, arrowtail=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname, color=edge_color)
                    else:
                        if random.random() >= 0.2:
    ...
    def generate_neato(self, n):
        ...
        for l in range(n):
            ...
            if len(self.latent_variables) % 2 == 0:
                positions = [[f'{(1 - n//2) + 2*i},{-1}!' if i < n//2 else f'{(1 - n//2) + 2*(i - n//2)},{1}!' for i in range(n)],
                             [f'{-1},{(1 - n//2) + 2*i}!' if i < n//2 else f'{1},{(1 - n//2) + 2*(i - n//2)}!' for i in range(n)]]
            else:
                positions = [[f'{(1 - n//2) + 2*i},{-1}!' if i < n//2 else f'{-(n//2) + 2*(i - n//2)},{1}!' for i in range(n)],
                             [f'{-1},{(1 - n//2) + 2*i}!' if i < n//2 else f'{1},{-(n//2) + 2*(i - n//2)}!' for i in range(n)],
                             [f'{(1 - n//2) + 2*i},{1}!' if i < n//2 else f'{-(n//2) + 2*(i - n//2)},{-1}!' for i in range(n)],
                             [f'{1},{(1 - n//2) + 2*i}!' if i < n//2 else f'{-1},{-(n//2) + 2*(i - n//2)}!' for i in range(n)]]
            ...
            for ind, lv in enumerate(self.latent_variables):
                ...
                self.dot.node(f'{lv}_construct', lv, pos=pos[ind], style='filled', fillcolor=fill_color, gradientangle=fill_angle, fontname=fontname, fontsize=fontsz)
                if error == 'both':
                    label = self._create_edge_label()
            ...
            if curr_positions % 2 == 0:
                ...
                e = self._place_neato_objects(self.dot, self.obs_vars, self.latent_variables, pos, vert_pos, f'{-amount_n//2+1},{1+v_dist}!', 
                                              'filled', fill_color, fill_angle, pc_sz, arrow, arrow_sz, arrow_style, width, amount_n, 'hor',
                                              error, error_shape, fontname=fontname)
                self._place_neato_objects(self.dot, self.obs_vars, self.latent_variables, pos, vert_pos, f'{-amount_s//2+1},{-1-v_dist}!', 
                                          'filled', fill_color, fill_angle, pc_sz, arrow, arrow_sz, arrow_style, width, amount_s, 'hor',
                ...
                # 2. distribute items
                v_dist = random.randint(2, 3)
                e = self._place_neato_objects(self.dot, self.obs_vars, self.latent_variables, pos, vert_pos, f'{1+v_dist},{-amount_e//2+1}!', 
                                              'filled', fill_color, fill_angle, pc_sz, arrow, arrow_sz, arrow_style, width, amount_e, 'ver',
                                              error, error_shape, fontname=fontname)
                self._place_neato_objects(self.dot, self.obs_vars, self.latent_variables, pos, vert_pos, f'{-1-v_dist},{-amount_w//2+1}!', 
                                          'filled', fill_color, fill_angle, pc_sz, arrow, arrow_sz, arrow_style, width, amount_w, 'ver',
                                          error, error_shape, e, fontname=fontname)
    ...
    def _place_neato_objects(self, dot_obj, obs_vars, latent_variables, pos, vert_pos, position, 
        ...
        for node in [ind for ind, pos in enumerate(vert_pos) if pos == filter_pos]:
            ...
            for var in obs_vars[node]:
                ...
                dot_obj.node(f'{var}_item', var, pos=position, style=style, fillcolor=color, gradientangle=angle, fontname=fontname)
                label = self._create_edge_label()
                if random.random() >= 0.2:
                    dot_obj.edge(f'{latent_variables[node]}_construct', f'{var}_item', label=label, fontsize=pc_sz, 
                                 arrowhead=arrow, arrowsize=arrow_sz, style=arrow_style, penwidth=width, fontname=fontname)
                else:
    ...
    def _create_edge_label(self):
        ...
        if random.random() >= 0.5:
            second_line = random.choice(['t', 'x', 'y', 'a', 's', 'H', 'h', 'f']) + " = " + str(round(random.random(), 3))
            label = f'''<
    ...
    def _edge_directions(self):
        if self.n2 > 2: 
            edge_lst = random.sample(list(itertools.combinations(self.latent_variables, 2)), random.randint(self.n1//2, self.n2-1)) 
        else:
        ...
        return [[edge[0], edge[1], 'both'] if random.random() < 0.3 else [edge[0], edge[1], 'forward'] for edge in edge_lst]
